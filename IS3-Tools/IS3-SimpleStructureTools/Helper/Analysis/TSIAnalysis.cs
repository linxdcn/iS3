...
namespace IS3.SimpleStructureTools.Helper.Analysis
{
    ...
    public class TSIAnalysis
    {
        ...
        public static List<RingTSI> getTSIResult(List<SegmentLining> sls)
        {
            ...
            List<SegmentLining> orderSLList = orderSL.ToList();
            ...
            foreach (SegmentLining sl in orderSLList)
            {
            ...
            foreach (SectionTSIInfo info in infos)
                info.tsi = 5.23 - 0.16 * Math.Sqrt(Math.Abs(info.s_ave)) - 0.01 * info.s_diff - 0.09 * info.c_ave
                    - 0.08 * info.leakage - 0.05 * info.crack - 0.5 * info.spall;

            foreach (SectionTSIInfo info in infos)
            {
                for (int i = info.startRingNo; i <= info.endRingNo; i++)
                {
                    ...
                    ringTSI.leakage = info.leakage;
                    ringTSI.crack = info.crack;
                    ringTSI.spall = info.spall;
                    results.Add(ringTSI);
                }
            }
            return results;
        ...
        private static void GetSpallResult(List<SegmentLining> spallRecords, List<SectionTSIInfo> infos)
        {
            foreach (SectionTSIInfo info in infos)
            {
                ...
                dateList.Sort();
                if (dateList.Count > 0)
                    info.spall = results[dateList.Last()] / Math.Abs(info.startRingNo - info.endRingNo) * 100;
                else
                    info.spall = 0;
            }
        }
